	...
	game, GameController = function() {
		function n() {
			...
			this.maleMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxMaleMoundSize() - this.maleMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大公主孵化队列
			this.maxPrincessMoundSize = ko.observable(1);
			this.princessMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxPrincessMoundSize() - this.princessMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大王子孵化队列
			this.maxPrinceMoundSize = ko.observable(1);
			this.princeMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxPrinceMoundSize() - this.princeMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大皇后孵化队列
			this.maxFemaleMoundSize = ko.observable(1);
			this.femaleMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxFemaleMoundSize() - this.femaleMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大草场工作队列
			this.maxFarmMoundSize = ko.observable(1);
			this.farmMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxFarmMoundSize() - this.farmMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大矿厂工作队列
			this.maxMineMoundSize = ko.observable(1);
			this.mineMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxMineMoundSize() - this.mineMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大运输员队列
			this.maxCarrierMoundSize = ko.observable(1);
			this.carrierMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxCarrierMoundSize() - this.carrierMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大加工厂队列
			this.maxFactoryMoundSize = ko.observable(1);
			this.factoryMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxFactoryMoundSize() - this.factoryMound().length; t++) n.push(null);
				return n
			}, this);
			// 最大军营士兵数量
			this.maxArmyMoundSize = ko.observable(1);
			this.armyMoundEmpty = ko.computed(function() {
				for (var n = [], t = 0; t < this.maxArmyMoundSize() - this.armyMound().length; t++) n.push(null);
				return n
			}, this);
			// 草场的每秒产出
			...
			for (var n = 0; n < 23; n++) this.achievementCounts.push(new AchievementCount(n));
			...
			//function n(enemy, custom, name, desc, lowBaseValue, highBaseValue, armySizeBase, requiredToUnlock, treasurePoints) {
		...
		return n.prototype.Tick = function() {
			...
			for (var t, i, r, u = 0, n = 0; n < this.farmMound().length; n++) u += this.farmMound()[n].grassPerSecond;
			for (this.grassPerSecondRaw(u * (1 + this.bonusFarmPercent() / 100)), this.achievementCounts[15].Update(this.grassPerSecondRaw()), t = 0, n = 0; n < this.mineMound().length; n++) t += this.mineMound()[n].dirtPerSecond;
			for (this.dirtPerSecondRaw(t * (1 + this.bonusMinePercent() / 100)), this.achievementCounts[9].Update(this.dirtPerSecondRaw()), i = 0, n = 0; n < this.carrierMound().length; n++) i += this.carrierMound()[n].carryPerSecond;
			for (this.carryPerSecondRaw(i * (1 + this.bonusCarrierPercent() / 100)), this.achievementCounts[17].Update(this.carryPerSecondRaw()), r = 0, n = 0; n < this.factoryMound().length; n++) r += this.factoryMound()[n].sodPerSecond;
			...
			/*
			if (n || this.father().currentHealth() >= this.father().health && this.mother().currentHealth() >= this.mother().health) {
				this.Breed(this.mother(), this.father(), "Royal");
				!n && this.boosts() < this.maxBoosts() && this.boosts(Math.round((this.boosts() + .1) * 10) / 10);
			} else {
				this.pauseBreeding() || (this.mother().currentHealth(
			...
			*/
			...
			if (n || this.father().currentHealth() >= this.father().health && this.mother().currentHealth() >= this.mother().health ? (this.Breed(this.mother(), this.father(), "Royal"), !n && this.boosts() < this.maxBoosts() && this.boosts(Math.round((this.boosts() + .1) * 10) / 10)) : this.pauseBreeding() || (this.mother().currentHealth(this.mother().currentHealth() + this.mother().health / this.mother().actionTime), this.father().currentHealth(this.father().currentHealth() + this.father().health / this.father().actionTime)), this.prince().currentHealth() >= this.prince().health && this.princess().currentHealth() >= this.princess().health) this.Breed(this.princess(), this.prince(), "Heir");
				...
				var r = this.sodRaw() / ticksPerSecond > this.sodPerSecondForBreeding() / ticksPerSecond ? this.sodPerSecondForBreeding() / ticksPerSecond : this.sodRaw() / ticksPerSecond,
					t = this.princess().score * 5,
					i = this.prince().score * 5;
			...
			for (n.currentHealth(0), t.currentHealth(0), c = n.generation > t.generation ? n.generation + 1 : t.generation + 1, c > this.generations() && this.generations(c), this.achievementCounts[7].Update(this.generations()), y = CoinFlip() ? 0 : 1, u = this.DefaultCritter(y, 1, c), r = 0; r < n.traits.length; r++) {
				for (u.traits[r].base = n.traits[r].base >= this.traitMax && t.traits[r].base >= this.traitMax ? this.traitMax : this.MutateStat(n.traits[r].base, t.traits[r].base, 1, this.traitMax), l = 0; l < n.traits[r].genes.length; l++) f = n.traits[r].genes[l], h = jQuery.grep(t.traits[r].genes, function(n) {
					return n.id == f.id
				...
				for (v = 0; v < t.traits[r].genes.length; v++) s = t.traits[r].genes[v], p = jQuery.grep(n.traits[r].genes, function(n) {
					return n.id == s.id
			...
			for (u.CalculateScore(), this.NewGene(u), r = 0; r < u.traits.length; r++) this.achievementCounts[r].Update(u.traits[r].value);
			...
			var e = RandomInRange(1, this.newGeneChanceRange),
				r, i, t, f, u;
			if (e <= this.newGeneChance())
				for (this.missNewGene(0), r = !1, i = 0; i < availableGenes.length; i++) {
					if (r) break;
					if (t = availableGenes[i], f = jQuery.grep(n.traits[t.trait].genes, function(n) {
							return n.id == t.id
						}), f.length == 0) {
						var o = n.traits[t.trait].base,
							s = n.traits[t.trait].bonus,
							h = n.traits[t.trait].genes.length + 1;
				...
				case "Worker":
					for (e = 0; e < o.length; e++) {
						...
						var h = u.dirtPerSecond > this.lowestMiner() || this.mineMound().length < this.maxMineMoundSize(),
							c = u.grassPerSecond > this.lowestFarmer() || this.farmMound().length < this.maxFarmMoundSize(),
							v = u.carryPerSecond > this.lowestCarrier() || this.carrierMound().length < this.maxCarrierMoundSize(),
							y = u.sodPerSecond > this.lowestFactory() || this.factoryMound().length < this.maxFactoryMoundSize(),
							s = Math.min(this.dirtPerSecondRaw(), this.grassPerSecondRaw(), this.carryPerSecondRaw(), this.sodPerSecondRaw());
			...
			var t = this.nation().mapComplete() ? this.nation().mapComplete() : confirm("Are you sure you want to end this war?  You haven\'t finished this map yet and you\'ll have to start over."),
				n;
			if (t) {
				if (this.atWar(!1), this.nation().isDefeated())
					for (n = 0; n < this.nations().length; n++) this.nations()[n].requiredToUnlock == this.nation().enemy && this.nations()[n].isUnlocked(!0);
				this.map().tiles([]);
				this.armyUpgrades(new ArmyUpgrades);
				this.Save()
			}
			...
			for (this.showTreasure(!1), this.inBattle(!0), this.map().CreateArmy(), n = 0; n < this.armyMound().length; n++) t = new BattleMoundIndex(this.armyMound()[n].id, !0, n, this.armyMound()[n].actionTime), this.battleOrder.push(t);
			for (n = 0; n < this.map().enemyArmyMound().length; n++) t = new BattleMoundIndex(this.map().enemyArmyMound()[n].id, !1, n, this.map().enemyArmyMound()[n].actionTime), this.battleOrder.push(t);
			...
			if (this.inBattle())
				if (this.battleTurnClock <= 0) {
					...
					if (this.battleAttackerId(this.battleAttacker.id), f = jQuery.grep(this.battleDefenderMound(), function(n) {
						...
						if (this.battleDefender = f[RandomInRange(0, f.length - 1)], this.battleDefenderId(this.battleDefender.id), r = 0, o = 1, this.battleAttackerMoundIndex.isPlayer && RandomInRange(1, 100) <= this.battleAttacker.level() && (o = 2), CoinFlip()) {
							var l = this.battleAttackerMoundIndex.isPlayer ? this.battleAttacker.traits[3].value * (1 + this.armyUpgrades().biteBonus() / 100) : this.battleAttacker.traits[3].value,
								a = this.battleAttackerMoundIndex.isPlayer ? this.battleDefender.traits[4].value : this.battleDefender.traits[4].value * (1 + this.armyUpgrades().stingBonus() / 100),
								v = this.battleAttackerMoundIndex.isPlayer ? this.battleAttacker.strengthBonus * (1 + this.armyUpgrades().strengthBonus() / 100) : this.battleAttacker.strengthBonus,
								y = this.battleAttackerMoundIndex.isPlayer ? this.battleDefender.agilityBonus : this.battleDefender.agilityBonus * (1 + this.armyUpgrades().agilityBonus() / 100);
							e = (l + v) * o;
							h = a + y;
							this.battleAttackerTrait(3);
							this.battleDefenderTrait(4)
						} else {
							var l = this.battleAttackerMoundIndex.isPlayer ? this.battleDefender.traits[3].value : this.battleDefender.traits[3].value * (1 + this.armyUpgrades().biteBonus() / 100),
								a = this.battleAttackerMoundIndex.isPlayer ? this.battleAttacker.traits[4].value * (1 + this.armyUpgrades().stingBonus() / 100) : this.battleAttacker.traits[4].value,
								v = this.battleAttackerMoundIndex.isPlayer ? this.battleDefender.strengthBonus : this.battleDefender.strengthBonus * (1 + this.armyUpgrades().strengthBonus() / 100),
								y = this.battleAttackerMoundIndex.isPlayer ? this.battleAttacker.agilityBonus * (1 + this.armyUpgrades().agilityBonus() / 100) : this.battleAttacker.agilityBonus;
							h = l + v;
							e = (a + y) * o;
						...
						if (this.battleAttackerMoundIndex.isPlayer) {
							if (this.map().battleReport.won(!0), c = this.map().Clear(), this.map().tilesCleared() == this.map().tileCount() && (this.nation().mapComplete(!0), this.achievementCounts[21].Update(this.achievementCounts[21].value + 1)), c != null) {
								switch (c) {
									case 0:
										this.nation().isDefeated(!0);
										this.treasureTitle("YOU HAVE DESTROYED THE ENEMY");
										this.treasureText("YOU HAVE WON THE WAR.  IS THERE MORE?");
										break;
									case 1:
										this.nation().mineFound() == !1 && (this.bonusMinePercent(this.bonusMinePercent() + 2), this.nation().mineFound(!0), this.treasureTitle("CAPTURED AN ENEMY MINE"), this.treasureText("YOUR CRITTERS CAN NOW MINE 2% FASTER"), this.achievementCounts[10].Update(this.achievementCounts[10].value + 1));
										break;
									case 2:
										this.nation().farmFound() == !1 && (this.bonusFarmPercent(this.bonusFarmPercent() + 2), this.nation().farmFound(!0), this.treasureTitle("CAPTURED AN ENEMY FARM"), this.treasureText("YOUR CRITTERS CAN NOW FARM 2% FASTER"), this.achievementCounts[18].Update(this.achievementCounts[18].value + 1));
										break;
									case 3:
										this.nation().carryFound() == !1 && (this.bonusCarrierPercent(this.bonusCarrierPercent() + 2), this.nation().carryFound(!0), this.treasureTitle("CAPTURED ENEMY EQUIPMENT"), this.treasureText("YOUR CRITTERS CAN CARRY 2% MORE"), this.achievementCounts[19].Update(this.achievementCounts[19].value + 1));
										break;
									case 4:
										this.nation().factoryFound() == !1 && (this.bonusFactoryPercent(this.bonusFactoryPercent() + 2), this.nation().factoryFound(!0), this.treasureTitle("CAPTURED AN ENEMY FACTORY"), this.treasureText("YOUR CRITTERS CAN NOW PRODUCE SOD 2% FASTER"), this.achievementCounts[20].Update(this.achievementCounts[20].value + 1));
										break;
									case 8:
										this.nation().exploreFound() == !1 && (this.nation().exploreFound(!0), this.treasureTitle("FOUND HIGH GROUND"), this.treasureText("YOUR EXPLORATION SPEED INCREASED 50% ON THIS MAP"), this.achievementCounts[12].Update(this.achievementCounts[12].value + 1));
										break;
									case 9:
										this.nation().fortFound() == !1 && (this.nation().fortFound(!0), this.treasureTitle("FOUND AN ABANDONED FORT"), this.treasureText("YOUR TRACKING SPEED INCREASED 50% ON THIS MAP"), this.achievementCounts[22].Update(this.achievementCounts[22].value + 1));
										break;
									case 7:
										this.nation().boostFound() == !1 && (this.maxBoosts(this.maxBoosts() + 5), this.boosts(this.maxBoosts()), this.nation().boostFound(!0), this.treasureTitle("LOVE CONQUERS ALL"), this.treasureText("YOUR MAXIMUM BOOSTS HAVE INCREASED BY 5."), this.achievementCounts[14].Update(this.achievementCounts[14].value + 1));
										break;
									case 6:
										for (i = enemyGenes[this.nation().enemy], t = 0; t < i.length; t++) u = new Gene(i[t].id, i[t].trait, i[t].expression, i[t].name, i[t].value, i[t].good), u.mutation = !0, b = jQuery.grep(this.mother().traits[i[t].trait].genes, function(n) {
											return n.id == i[t].id
										...
										break;
									case 5:
										this.nation().treasuresFound(this.nation().treasuresFound() + 1);
										this.treasureTitle("YOU FOUND AN ARTIFACT");
										w = RandomInRange(1, 4);
										switch (w) {
											case 1:
												this.treasureText("IT IS INCREASING YOUR TROOPS STRENGTH");
												this.armyUpgrades().strengthBonus(this.armyUpgrades().strengthBonus() + 5);
												break;
											case 2:
												this.treasureText("IT IS INCREASING YOUR TROOPS AGILITY");
												this.armyUpgrades().agilityBonus(this.armyUpgrades().agilityBonus() + 5);
												break;
											case 3:
												this.treasureText("IT IS INCREASING YOUR TROOPS BITE");
												this.armyUpgrades().biteBonus(this.armyUpgrades().biteBonus() + 5);
												break;
											case 4:
												this.treasureText("IT IS INCREASING YOUR TROOPS STING");
				...
				for (n = 0; n < this.armyMound().length; n++) this.armyMound()[n].currentHealth() < this.armyMound()[n].health && (this.armyMound()[n].currentHealth(this.armyMound()[n].currentHealth() + this.armyMound()[n].health / this.armyMound()[n].actionTime * (1 + this.armyUpgrades().medicBonus() / 100)), this.armyMound()[n].currentHealth() > this.armyMound()[n].health && this.armyMound()[n].currentHealth(this.armyMound()[n].health))
		}, n.prototype.UpdateArmyUpgrades = function() {
			var t, n;
			for (this.armyUpgrades().generalBonus(0), this.armyUpgrades().scoutBonus(0), this.armyUpgrades().medicBonus(0), this.armyUpgrades().hasGeneral(!1), this.armyUpgrades().hasScout(!1), this.armyUpgrades().hasMedic(!1), t = 0, n = 0; n < this.armyMound().length; n++) switch (t) {
				case 0:
					this.armyMound()[n].rank("General");
					this.armyUpgrades().hasGeneral(!0);
					this.armyUpgrades().generalBonus(this.armyMound()[n].level() * 10);
					t++;
					break;
				case 1:
					this.armyMound()[n].rank("Scout");
					this.armyUpgrades().hasScout(!0);
					this.armyUpgrades().scoutBonus(this.armyMound()[n].level() * 10);
					t++;
					break;
				case 2:
					this.armyMound()[n].rank("Medic");
					this.armyUpgrades().hasMedic(!0);
					this.armyUpgrades().medicBonus(this.armyMound()[n].level() * 10);
					t++;
					break;
				default:
					this.armyMound()[n].rank(this.armyMound()[n].level() < 8 ? "Soldier" : this.armyMound()[n].level() < 15 ? "Veteran" : "Elite")
			}
			...
			if (this.achievementCheck <= 0) {
				for (var n = 0; n < this.achievements().length; n++) this.achievementCounts[this.achievements()[n].type].value >= this.achievements()[n].value && !this.achievements()[n].isUnlocked() && (this.achievements()[n].isUnlocked(!0), this.achievementsUnlocked(this.achievementsUnlocked() + 1), $(".tabcontents").notify("Achievement Unlocked. " + this.achievements()[n].name, "info"));
				this.achievementCheck = 5 * ticksPerSecond
			} else this.achievementCheck--
		}, n.prototype.AnimateWorkers = function() {
			for (var n = 0; n < this.mineMound().length; n++) this.mineMound()[n].currentHealth(this.mineMound()[n].currentHealth() + this.mineMound()[n].health / this.mineMound()[n].actionTime), this.mineMound()[n].currentHealth() >= this.mineMound()[n].health && this.mineMound()[n].currentHealth(0);
			for (n = 0; n < this.farmMound().length; n++) this.farmMound()[n].currentHealth(this.farmMound()[n].currentHealth() + this.farmMound()[n].health / this.farmMound()[n].actionTime), this.farmMound()[n].currentHealth() >= this.farmMound()[n].health && this.farmMound()[n].currentHealth(0);
			for (n = 0; n < this.carrierMound().length; n++) this.carrierMound()[n].currentHealth(this.carrierMound()[n].currentHealth() + this.carrierMound()[n].health / this.carrierMound()[n].actionTime), this.carrierMound()[n].currentHealth() >= this.carrierMound()[n].health && this.carrierMound()[n].currentHealth(0);
			for (n = 0; n < this.factoryMound().length; n++) this.factoryMound()[n].currentHealth(this.factoryMound()[n].currentHealth() + this.factoryMound()[n].health / this.factoryMound()[n].actionTime), this.factoryMound()[n].currentHealth() >= this.factoryMound()[n].health && this.factoryMound()[n].currentHealth(0)
			...
			for (var t = this.maleMound().length, n; this.maleMound().length > this.maxMaleMoundSize();) {
			...
			for (n = this.femaleMound().length; this.femaleMound().length > this.maxFemaleMoundSize();) {
			...
			for (t = this.princeMound().length; this.princeMound().length > this.maxPrinceMoundSize();) {
			...
			for (n = this.princessMound().length; this.princessMound().length > this.maxPrincessMoundSize();) {
			...
			for (this.SortMound(this.maleMound, this.maleSort()), this.SortMound(this.femaleMound, this.femaleSort()), this.SortMound(this.princeMound, this.princeSort()), this.SortMound(this.princessMound, this.princessSort()), this.SortMound(this.carrierMound, "carry"), this.SortMound(this.mineMound, "mine"), this.SortMound(this.farmMound, "farm"), this.SortMound(this.factoryMound, "factory"), this.SortMound(this.armyMound, this.armySort()); this.carrierMound().length > this.maxCarrierMoundSize();) this.carrierMound.pop();
			...
			var f = n < t ? n - StatVariance(n) : t - StatVariance(t),
				e = n > t ? n + StatVariance(n) : t + StatVariance(t),
				u = RandomInRange(f, e);
			...
			try {
				if (n !== null || localStorage.getItem("game2") !== null) {
					if (t = n != null ? JSON.parse($.base64.decode(n)) : JSON.parse($.base64.decode(localStorage.getItem("game2"))), this.dirtRaw(t.dirtRaw), this.grassRaw(t.grassRaw), this.sodRaw(t.sodRaw), this.factoryDirtRaw(t.factoryDirtRaw), this.factoryGrassRaw(t.factoryGrassRaw), this.generations(t.generations), this.sodDedicatedToBreeding(t.sodDedicatedToBreeding), this.isHeirsUnlocked(t.isHeirsUnlocked), this.maxFemaleMoundSize(t.maxFemaleMoundSize), this.maxMaleMoundSize(t.maxMaleMoundSize), this.maxPrincessMoundSize(t.maxPrincessMoundSize), this.maxPrinceMoundSize(t.maxPrinceMoundSize), this.maxMineMoundSize(t.maxMineMoundSize), this.maxFarmMoundSize(t.maxFarmMoundSize), this.maxCarrierMoundSize(t.maxCarrierMoundSize), this.maxFactoryMoundSize(t.maxFactoryMoundSize), this.maxArmyMoundSize(t.maxArmyMoundSize), this.bonusMinePercent(t.bonusMinePercent), this.bonusCarrierPercent(t.bonusCarrierPercent), this.bonusFarmPercent(t.bonusFarmPercent), this.bonusFactoryPercent(t.bonusFactoryPercent), this.femaleSort(t.femaleSort), this.maleSort(t.maleSort), this.princeSort(t.princeSort), this.princessSort(t.princessSort), this.armySort(t.armySort), this.atWar(t.atWar), this.achievementsUnlocked(t.achievementsUnlocked), this.battleTurnLength(t.battleTurnLength != undefined ? t.battleTurnLength : ticksPerSecond / 2), t.boosts != undefined && (this.boosts(t.boosts), this.maxBoosts(t.maxBoosts)), t.nations != undefined)
						for (this.nations = ko.observableArray(), i = 0; i < t.nations.length; i++) u = new Nation(t.nations[i].enemy, t.nations[i].custom, t.nations[i].name, t.nations[i].desc, t.nations[i].lowBaseValue, t.nations[i].highBaseValue, t.nations[i].armySizeBase, t.nations[i].requiredToUnlock, t.nations[i].treasurePoints), u.mineFound(t.nations[i].mineFound), u.farmFound(t.nations[i].farmFound), u.carryFound(t.nations[i].carryFound), u.factoryFound(t.nations[i].factoryFound), u.exploreFound(t.nations[i].exploreFound), u.fortFound(t.nations[i].fortFound != undefined ? t.nations[i].fortFound : !1), u.geneFound(t.nations[i].geneFound), u.boostFound(t.nations[i].boostFound), u.treasuresFound(t.nations[i].treasuresFound), u.mapComplete(t.nations[i].mapComplete), u.isDefeated(t.nations[i].isDefeated), u.isUnlocked(t.nations[i].isUnlocked), this.nations.push(u);
					if (t.map != undefined)
						for (this.map(new GameMap), this.map().mound = t.map.mound, this.map().enemy = t.map.enemy, this.map().mine = t.map.mine, this.map().farm = t.map.farm, this.map().carry = t.map.carry, this.map().factory = t.map.factory, this.map().explore = t.map.explore, this.map().boost = t.map.boost, this.map().gene = t.map.gene, this.map().fort = t.map.fort, this.map().treasures = t.map.treasures, this.map().tileCount(t.map.tileCount), this.map().tilesCleared(t.map.tilesCleared), this.map().canExplore(t.map.canExplore), this.map().highestDanger = t.map.highestDanger != undefined ? t.map.highestDanger : 1, i = 0; i < t.tiles.length; i++) {
							for (s = ko.observableArray(), f = 0; f < t.tiles[i].length; f++) e = new GameMapTile(new Point(f, i)), e.isCleared(t.tiles[i][f].isCleared), e.isUnlocked(t.tiles[i][f].isUnlocked), e.extraClass(t.tiles[i][f].extraClass), e.danger(t.tiles[i][f].danger), s.push(e);
							this.map().tiles.push(s)
						}
					if (t.nation != undefined && (o = jQuery.grep(this.nations(), function(n) {
							return n.enemy == t.nation.enemy
						}), o.length > 0 && (this.nation(o[0]), this.map().nation = o[0])), t.achievements != undefined)
						for (i = 0; i < t.achievements.length; i++) this.achievements()[i].isUnlocked(t.achievements[i].isUnlocked);
					if (t.achievementCounts != undefined) {
						for (this.achievementCounts = [], i = 0; i < t.achievementCounts.length; i++) h = new AchievementCount(t.achievementCounts[i].type), h.value = t.achievementCounts[i].value, this.achievementCounts.push(h);
						while (this.achievementCounts.length < 23) this.achievementCounts.push(new AchievementCount(i))
					}
					for (r = new Critter(t.mother.generation, t.mother.id, t.mother.gender), r.Load(t.mother), this.mother(r), r = new Critter(t.father.generation, t.father.id, t.father.gender), r.Load(t.father), this.father(r), r = new Critter(t.princess.generation, t.princess.id, t.princess.gender), r.Load(t.princess), this.princess(r), r = new Critter(t.prince.generation, t.prince.id, t.prince.gender), r.Load(t.prince), this.prince(r), this.femaleMound.removeAll(), i = 0; i < t.femaleMound.length; i++) r = new Critter(t.femaleMound[i].generation, t.femaleMound[i].id, t.femaleMound[i].gender), r.Load(t.femaleMound[i]), this.femaleMound.push(r);
					for (this.maleMound.removeAll(), i = 0; i < t.maleMound.length; i++) r = new Critter(t.maleMound[i].generation, t.maleMound[i].id, t.maleMound[i].gender), r.Load(t.maleMound[i]), this.maleMound.push(r);
					for (this.princessMound.removeAll(), i = 0; i < t.princessMound.length; i++) r = new Critter(t.princessMound[i].generation, t.princessMound[i].id, t.princessMound[i].gender), r.Load(t.princessMound[i]), this.princessMound.push(r);
					for (this.princeMound.removeAll(), i = 0; i < t.princeMound.length; i++) r = new Critter(t.princeMound[i].generation, t.princeMound[i].id, t.princeMound[i].gender), r.Load(t.princeMound[i]), this.princeMound.push(r);
					for (this.mineMound.removeAll(), i = 0; i < t.mineMound.length; i++) r = new Critter(t.mineMound[i].generation, t.mineMound[i].id, t.mineMound[i].gender), r.Load(t.mineMound[i]), this.mineMound.push(r);
					for (this.farmMound.removeAll(), i = 0; i < t.farmMound.length; i++) r = new Critter(t.farmMound[i].generation, t.farmMound[i].id, t.farmMound[i].gender), r.Load(t.farmMound[i]), this.farmMound.push(r);
					for (this.carrierMound.removeAll(), i = 0; i < t.carrierMound.length; i++) r = new Critter(t.carrierMound[i].generation, t.carrierMound[i].id, t.carrierMound[i].gender), r.Load(t.carrierMound[i]), this.carrierMound.push(r);
					for (this.factoryMound.removeAll(), i = 0; i < t.factoryMound.length; i++) r = new Critter(t.factoryMound[i].generation, t.factoryMound[i].id, t.factoryMound[i].gender), r.Load(t.factoryMound[i]), this.factoryMound.push(r);
					for (this.armyMound.removeAll(), i = 0; i < t.armyMound.length; i++) r = new Critter(t.armyMound[i].generation, t.armyMound[i].id, t.armyMound[i].gender), r.Load(t.armyMound[i]), this.armyMound.push(r);
					this.armyUpgrades(new ArmyUpgrades);
	...
	Critter = function() {
		...
		return n.prototype.Load = function(n) {
			...
			var r = 1,
				...
				f = 1,
				n, t, i, e, o;
			for (this.totalMutations = 0, n = 0; n < this.traits.length; n++) {
				for (t = 0, i = 0; i < this.traits[n].genes.length; i++) this.traits[n].genes[i].expression == 2 && (this.traits[n].genes[i].good ? t += this.traits[n].genes[i].value : t -= this.traits[n].genes[i].value), this.totalMutations++;
				t = Math.round(t);
		...
		return n.prototype.GetTile = function(n) {
			...
			if (this.currentBattle != null && (n = this.GetTile(this.currentBattle), n.isUnlocked())) {
				...
				for (r = 0; r < this.treasures.length; r++)
					if (this.treasures[r] != undefined && n.coords.x == this.treasures[r].x && n.coords.y == this.treasures[r].y) return n.extraClass("treasure"), n.danger(0), 5
			}
			...
			for (var r = 0; r < t.length; r++)
				if (n.x == t[r].x && n.y == t[r].y || this.Distance(n, t[r]) < i) return !0;
			return !1
			...
			var f = RandomInRange(u - 1, u + 2),
				e = this.Distance(n, this.mound),
				o = this.Distance(n, this.enemy),
				c = this.mine != undefined ? this.Distance(n, this.mine) : 999,
				l = this.farm != undefined ? this.Distance(n, this.farm) : 999,
				a = this.carry != undefined ? this.Distance(n, this.carry) : 999,
				v = this.factory != undefined ? this.Distance(n, this.factory) : 999,
				s = Math.min(c, l, a, v);
			...
			if (this.canExplore) {
				if (n == null) {
					for (r = [], i = 0; i < this.tiles().length; i++) {
						for (f = 0; f < this.tiles()[i]().length; f++)
							if (u = this.GetTile(new Point(f, i)), u.isUnlocked() && !u.isCleared() && u.danger() == 0 && u.extraClass() == "" && r.push(u), r.length >= 25) break;
				...
				for (i = 0; i < this.treasures.length; i++)
					if (this.treasures[i] != undefined && t.coords.x == this.treasures[i].x && t.coords.y == this.treasures[i].y) {
			...
			for (var f, n, e, t = [], i = [], r = [], u = 0; u < this.tiles().length; u++)
				for (f = 0; f < this.tiles()[u]().length; f++) n = this.GetTile(new Point(f, u)), n.isUnlocked() && !n.isCleared() && n.danger() <= this.highestDanger && n.danger() > 0 && t.push(n), n.isUnlocked() && !n.isCleared() && n.danger() == 0 && n.extraClass() == "" && i.push(n), n.isUnlocked() && !n.isCleared() && n.danger() > this.highestDanger && r.push(n);
			...
			if (this.currentBattle != null) {
				...
				var s = this.BaseStat(o),
					u = s,
					n = s,
					t = s,
					i = s,
					r = s,
					f = this.nation.armySizeBase + Math.floor(o / 5);
				switch (this.nation.enemy) {
					case 0:
						this.battleReport.enemy("Swarm of Gnats");
						break;
					case 9:
						this.battleReport.enemy("Battalion of Beetles");
						break;
					case 4:
						this.battleReport.enemy("Bed of Scorpions");
						break;
					case 6:
						this.battleReport.enemy("Tarantula Warrior");
						break;
					case 3:
						this.battleReport.enemy("Nest of Wasps");
						break;
					case 12:
						this.battleReport.enemy("Plague of Ticks");
						break;
					case 17:
						this.battleReport.enemy("Cloud of Grasshoppers");
						break;
					case 2:
						this.battleReport.enemy("Swarm of Chiggers");
						break;
					case 1:
						this.battleReport.enemy("Quiver of Ladybugs");
						break;
					case 5:
						this.battleReport.enemy("Hive of Bees");
						break;
					case 11:
						this.battleReport.enemy("Nest of Termites");
						break;
					case 10:
						this.battleReport.enemy("Stable of Horseflys");
						break;
					case 16:
						this.battleReport.enemy("Colony of Ants");
						break;
					case 8:
						this.battleReport.enemy("Centipede Gladiator");
						break;
					case 7:
						this.battleReport.enemy("Praying Mantis Assassian");
						break;
					case 15:
						this.battleReport.enemy("Army of Crickets");
						break;
					case 14:
						this.battleReport.enemy("Lair of Leeches");
						break;
					case 13:
						this.battleReport.enemy("Scourge of Mosquitos")
				}
				...
				for (this.enemyArmyMound.removeAll(), l = 1; l <= f; l++) e = new Critter(s, -l, CoinFlip() ? 1 : 0), e.job = 4, e.traits[0].base = SmartRound(RandomInRange(u - StatVariance(u), u + StatVariance(u))), e.traits[1].base = SmartRound(RandomInRange(r - StatVariance(r), r + StatVariance(r))), e.traits[2].base = SmartRound(RandomInRange(i - StatVariance(i), i + StatVariance(i))), e.traits[3].base = SmartRound(RandomInRange(n - StatVariance(n), n + StatVariance(n))), e.traits[4].base = SmartRound(RandomInRange(t - StatVariance(t), t + StatVariance(t))), e.experience(c), e.CalculateScore(), this.enemyArmyMound.push(e)
			}
		}, n.prototype.Generate = function(n, t) {
			var o, f, s, a, h, c, r, u, i, e, l;
			for (this.tiles.removeAll(), o = 0, f = 0; f < t; f++) {
				for (s = ko.observableArray(), u = 0; u < n; u++) a = new GameMapTile(new Point(u, f)), s.push(a), o++;
				this.tiles.push(s)
			}
			if (this.tileCount(o), h = new Point(RandomInRange(4, n - 4), 3), c = new Point(RandomInRange(4, n - 4), t - 3), CoinFlip() ? (this.mound = h, this.enemy = c) : (this.mound = c, this.enemy = h), r = [], r.push(this.mound), r.push(this.enemy), !this.nation.mineFound())
				for (i = !1; !i;)(this.mine = new Point(RandomInRange(2, n - 3), RandomInRange(2, t - 3)), this.LocationTaken(this.mine, r, 5)) || (i = !0, r.push(this.mine));
			if (!this.nation.farmFound())
				for (i = !1; !i;)(this.farm = new Point(RandomInRange(2, n - 3), RandomInRange(2, t - 3)), this.LocationTaken(this.farm, r, 5)) || (i = !0, r.push(this.farm));
			if (!this.nation.carryFound())
				for (i = !1; !i;)(this.carry = new Point(RandomInRange(2, n - 3), RandomInRange(2, t - 3)), this.LocationTaken(this.carry, r, 5)) || (i = !0, r.push(this.carry));
			if (!this.nation.factoryFound())
				for (i = !1; !i;)(this.factory = new Point(RandomInRange(2, n - 3), RandomInRange(2, t - 3)), this.LocationTaken(this.factory, r, 5)) || (i = !0, r.push(this.factory));
			if (!this.nation.geneFound())
				for (i = !1; !i;)(this.gene = new Point(RandomInRange(0, n - 1), RandomInRange(0, t - 1)), this.LocationTaken(this.gene, r, 5)) || (i = !0, r.push(this.gene));
			if (!this.nation.boostFound())
				for (i = !1; !i;)(this.boost = new Point(RandomInRange(1, n - 2), RandomInRange(1, t - 2)), this.LocationTaken(this.boost, r, 5)) || (i = !0, r.push(this.boost));
			if (!this.nation.fortFound())
				for (i = !1; !i;)(this.fort = new Point(RandomInRange(1, n - 2), RandomInRange(1, t - 2)), this.LocationTaken(this.fort, r, 5)) || (i = !0, r.push(this.fort));
			if (!this.nation.exploreFound())
				for (i = !1; !i;)(this.explore = new Point(RandomInRange(1, n - 2), RandomInRange(1, t - 2)), this.LocationTaken(this.explore, r, 5)) || (i = !0, r.push(this.explore));
			for (u = 0; u < 4; u++)
				for (i = !1; !i;)(e = new Point(RandomInRange(0, n - 1), RandomInRange(0, t - 1)), this.LocationTaken(e, r, 4)) || (r.push(e), this.treasures.push(e), i = !0);
